# prod-bj outputs —— 暴露给 Task 2 overlay ConfigMap（PG_HOST / REDIS_HOST / KAFKA_BROKERS / kubeconfig）
# output 名（左侧）按 brief 固定；value 引用的字段名以各 module 实际 outputs.tf 为准（已对齐）

output "kubeconfig" {
  description = "prod-bj ACK kubeconfig"
...
output "peering_id" {
  description = "VPC Peering ID（与上海 prod 互通）"
