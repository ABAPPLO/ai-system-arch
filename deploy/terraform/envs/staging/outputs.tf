# staging outputs —— 暴露给 Task 2 overlay ConfigMap（PG_HOST / REDIS_HOST / KAFKA_BROKERS / kubeconfig）
# output 名（左侧）按 brief 固定，与 prod 对齐；value 引用的字段名以各 module 实际 outputs.tf 为准（已对齐）

output "kubeconfig" {
  description = "staging ACK kubeconfig"
