...
## E. 全 HTTP e2e（/dispatch,顺序 1w）

> `benchmarks/e2e_load.py` · 真鉴权(X-Ingress-Auth → Redis 身份读,走 R3e L1 路径);resolve/forward/kafka stub(隔离身份读成本);ASGI in-process。

| | p50 | p99 |
|---|---|---|
| L1 on(命中) | ~1.3 ms | ~1.8 ms |
| L1 off(每请求 Redis 身份读) | ~1.5 ms | ~3–4 ms |
| **Δ(L1 省)** | **~0.3 ms** | **~1–2 ms（p99 ~2×）** |

**结论**:全 HTTP 路径(中间件 + 鉴权 + forward)下,L1 仍可测地省下身份 Redis 读那 ~0.3ms p50 / ~1–2ms p99 —— 与 component 基准一致(身份读 ~120µs 量级,叠加 R3e 命中态省下的连接竞争)。**身份读是 ~1.3ms 请求里可测的一块,L1 把它清零。**

**方法学注意（重要）**:
- **顺序 1 worker** 量真 per-request 成本;**20 并发 ASGI in-process 会饱和事件循环(~22ms/请求,非代表性)**——生产并发数须用真 uvicorn server + 真 TCP 负载生成(vegeta/k6)量,ASGI harness 不适用。
- resolve/forward/kafka stub 掉了(它们非 R3e 变量);真实请求还会叠加 backend RTT + 真 resolve。
- 本地 docker Redis RTT 为下界。

**复现**:`PYTHONPATH=services/services/dispatcher/src:. .venv/bin/python -m benchmarks.e2e_load`(需 apihub-redis@:16380)。
