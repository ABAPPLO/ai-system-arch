...
> **[R3d 2026-07-21 评估 → §9-C 已知接受]**：post-R1d/R3a 实际链 = **2 HTTP + 2 Redis RTT**（非 4 串行 HTTP——auth 跳被 R1d Redis 身份缓存消除 warm path，quota 跳从未接入 sync dispatcher 热路径，每请求限流走 edge APISIX `limit-count` in-process Lua）。sidecar（Envoy `ext_authz` ~1-5ms gRPC）比当前 APISIX in-process Lua（~0.1-0.5ms）**慢一个量级** + 加部署/监控/扩容复杂度。R1d/R3a 已吸收原始张力；残余 2 Redis RTT 用 pipelining + dispatcher L1 消减（低风险增量，非 sidecar）。auth/quota 已在 edge（key-auth + limit-count Lua 插件）= sidecar 要 host 的功能已在最优位置。**结论：sidecar 不值得，§9-C 标已知接受。** 详见 [R3d 可行性报告](superpowers/specs/2026-07-21-r3d-sidecar-feasibility.md)。
