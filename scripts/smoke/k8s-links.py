...
"""
    ...
    cip = sh(
        f"kubectl -n {NAMESPACE} get svc mock-backend "
        f"-o jsonpath='{{.spec.clusterIP}}'", check=False,
    ).strip()
...
def link1_sync():
    ...
    try:
        ...
        st, body = http("POST", f"http://127.0.0.1:18001{path}",
...
def link4_admin():
    ...
    p = pf("admin", 18006)
    try:
        st, body = http("GET", "http://127.0.0.1:18006/v1/admin/dashboard",
                        headers={"X-API-Key": ADMIN_KEY})
